...
{
    public new EarthquakeAbilityData data => (EarthquakeAbilityData)base.data;
    ...
    void GetAffectedUnits() {
        ...
        foreach (var tile in radius) {
            if (tile.unit && tile.unit != unit && !unitsFound.Contains(tile.unit)) {
                ...
                tile.unit.gameObject.AddDataComponent(data.slowModifier2);
            }
        }
        ...
        foreach (var tile in radius) {
            if (tile.unit && tile.unit != unit && !unitsFound.Contains(tile.unit)) {
                ...
                tile.unit.gameObject.AddDataComponent(data.slowModifier3);
            }
        }
        unitsFound.Clear();
    }
